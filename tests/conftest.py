...
def auth_api_utils_anonym():
    api_utils = ApiUtils(AuthService.SERVICE_URL)
    return api_utils


@pytest.fixture(scope="function", autouse=False)
def university_api_utils_anonym():
    api_utils = ApiUtils(UniversityService.SERVICE_URL)
    return api_utils
...
@pytest.fixture(scope="function", autouse=False)
def admin_auth_api_utils(access_token):
    admin_api_utils = ApiUtils(AuthService.SERVICE_URL,
                               headers={"Authorization": f"Bearer {access_token}"})
    return admin_api_utils


@pytest.fixture(scope="function", autouse=False)
def admin_university_api_utils(access_token):
    admin_api_utils = ApiUtils(UniversityService.SERVICE_URL,
                               headers={"Authorization": f"Bearer {access_token}"})
    return admin_api_utils

@pytest.fixture(scope="session", autouse=True)
def auto_service_readiness():
    timeout = 180
