...
class ApiUtils:
    ...
    @log_response
    def delete(self, endpoint_url, item, **kwargs):
        response = self.session.delete(self.url + endpoint_url + f"{item}/", **kwargs)
        return response

    @log_response
    def get_with_id(self, endpoint_url, item, **kwargs):
        response = self.session.get(self.url + endpoint_url + f"{item}/", **kwargs)
        return response

    @log_response
    def get_with_query(self, endpoint_url, queries, **kwargs):
        response = self.session.get(self.url + endpoint_url, params=queries, **kwargs)
        return response

    @log_response
    def put(self, endpoint_url, item, data=None, **kwargs):
        response = self.session.put(self.url + endpoint_url + f"{item}/", data, **kwargs)
        return response
